...
# === Step 1: Alignment, deduplication, filtering ===
for folder in "$DATA_PATH"/*; do
    if [ -d "$folder" ]; then
        ...
        # Align reads
        bowtie2 --phred33 -X 2000 \
            -x "$INDEX_PATH" \
            -1 "$folder/${SAMPLE}_R1_paired.fq.gz" \
            -2 "$folder/${SAMPLE}_R2_paired.fq.gz" \
            -p "$THREADS" \
            -S "$folder/${SAMPLE}.sam"

        # SAM → BAM, filter low-quality/secondary reads
        samtools view -b -h -F 3852 "$folder/${SAMPLE}.sam" -o "$folder/${SAMPLE}.bam"
        rm "$folder/${SAMPLE}.sam"

        # Sort BAM
        samtools sort "$folder/${SAMPLE}.bam" -o "$folder/${SAMPLE}_sorted.bam"
        rm "$folder/${SAMPLE}.bam"

        # Remove duplicates
        picard MarkDuplicates \
            I="$folder/${SAMPLE}_sorted.bam" \
            O="$folder/${SAMPLE}_dedup.bam" \
            M="$folder/${SAMPLE}_dup_metrics.txt" \
            REMOVE_DUPLICATES=true
        rm "$folder/${SAMPLE}_sorted.bam"

        # Filter blacklist
        bedtools intersect -v -abam "$folder/${SAMPLE}_dedup.bam" -b "$BLACKLIST" > "$folder/${SAMPLE}_dedup_filtered.bam"
        rm "$folder/${SAMPLE}_dedup.bam"

        # Filter for MAPQ ≥30, non-mito, non-Y, properly paired
        samtools view -b -q 30 -f 2 "$folder/${SAMPLE}_dedup_filtered.bam" \
            | samtools view -h - \
            | awk 'BEGIN{OFS="\t"} /^@/ || ($3!="chrM" && $3!="chrY")' \
            | samtools view -b - > "$folder/${SAMPLE}_final.bam"
        rm "$folder/${SAMPLE}_dedup_filtered.bam"

        # Index final BAM
        samtools index "$folder/${SAMPLE}_final.bam"
